...
const getRoomIcon = (hasUsers: boolean) => {
  // Since Room type doesn't have a 'type' property, determine based on users
  if (hasUsers) {
    return <Users size={16} class='text-blue-600' />; // Room with users
  } else {
    return <Globe size={16} class='text-green-600' />; // Open room
  }
};
...
export const RoomApplicationCard: Component<Props> = (props) => {
  ...
  const getRoomTypeColor = () => {
    if (hasUsers()) {
      return "border-blue-200 bg-blue-50/30"; // Room with users
    } else {
  ...
  return (
    <div
      ...
      <div class='flex items-start justify-between mb-4'>
        ...
        {/* Remove featured and verified badges since they don't exist in Room type */}
      </div>
      ...
      <div class='flex gap-2'>
        <button
          class='w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg text-sm font-medium hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-[1.02] group/join'
          onClick={() => onApply(room.RoomId)}>
          <UserPlus
            size={14}
